"""high-poker-2 — Poker44 (SN126) bot-detection miner entrypoint.
...
class MLMiner(BaseMinerNeuron):
    """high-poker-2: tail-lens + lossguide-stack4 + monotone-lgbm + pca56-mlp80 (weighted-rank 35/30/35)."""
    ...
    def __init__(self, config=None):
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            defaults={
                "model_name": "high-poker-2",
                "model_version": "3.2",
                "framework": "tail-lens + lossguide-stack4 + monotone-lgbm + pca56-mlp80 (weighted-rank 35/30/35)",
                "license": "MIT",
                "repo_url": repo_url,
                ...
                "notes": ("Weighted-rank trio with a leaf-wise stack (LightGBM-127/lossguide-XGBoost/deep CatBoost-d7/RandomForest-d18 -> logistic meta), a 3-seed monotone-constrained LightGBM, and a 3-seed PCA(56)->MLP(80) member; tightest FPR target of the family (0.035). "
                          "Feature surface adds a tail lens: per-metric support range, upper/lower tail stretch and coefficient of variation, so this miner reads distribution edges rather than the middle. "
        ...
        bt.logging.info(
            f"high-poker-2 ready | cv_ap={meta.get('cv_ap', 0.0):.4f} "
    ...
    async def forward(self, synapse: DetectionSynapse) -> DetectionSynapse:
        ...
        record_synapse(synapse, miner="high-poker-2")
        return synapse
...
if __name__ == "__main__":
    with MLMiner() as miner:
        bt.logging.info("high-poker-2 miner running...")
        while True:
            try:
                bt.logging.info(
